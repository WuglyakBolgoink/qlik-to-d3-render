define(['jquery'], (function (t) {
    return function (t) {
        ...
        function (cssChartClass, _chartOptions, domEl) {
            'use strict';

            domEl.r(_chartOptions);

            var r = domEl(4),
                i = domEl.n(r),
                o = domEl(37),
                u = domEl.n(o),
                a = (domEl(114), 'invalid-visualisation-message');

            var drawRadar = function (cssChartClass, chartOptions, _domEl, qLayoutObj, _dimList, _qlikUI) {
                if (_dimList) {
                    ...
                    const f = _dimList.map((function (t) {
                        return t.definition;
                    }));

                    if (void 0 !== chartOptions) {
                        for (var h in chartOptions) {
                            void 0 !== chartOptions[h] && (chartDefaultOptions[h] = chartOptions[h]);
                        }
                    }
                    ...
                    const maxChartValue = Math.max(chartDefaultOptions.maxValue, d3.max(f, (function (t) {
                        return d3.max(t.map((function (t) {
                            return t.value;
                        })));
                    })));

                    const minChartValue = Math.min(0, d3.min(f, (function (t) {
                        return d3.min(t.map((function (t) {
                            return t.value;
                        })));
                    })));

                    !1 === qLayoutObj.range && (maxChartValue = isNaN(qLayoutObj.maxValue)
                        ? 1
                    ...
                    var allAxis = f[0].map((function (t) {
                        return t.axis;
                    }));

                    allAxis.length > 100 && (allAxis = allAxis.slice(0, 100));

                    var total = allAxis.length,
                        radius = Math.abs(Math.min(v / 2 - chartDefaultOptions.margin.left - chartDefaultOptions.margin.right, p / 2 - chartDefaultOptions.margin.top - chartDefaultOptions.margin.bottom)),
                        angleSlice = 2 * Math.PI / total,
                        rScale = d3.scale.linear().range([0, radius]).domain([minChartValue, maxChartValue]),
                        w = function (t) {
                    ...
                    d3.select(cssChartClass).select('svg').remove();

                    var chartContainerId = 'container_' + qLayoutObj.qInfo.qId;

                    _domEl.empty().append(u()('<div />').attr('id', chartContainerId).width(chartDefaultOptions.size.width).height(chartDefaultOptions.size.height));

                    var S = d3.select('#'.concat(chartContainerId)).append('svg').attr('width', chartDefaultOptions.size.width).attr('height', chartDefaultOptions.size.height).classed('in-edit-mode', _qlikUI._inEditState),
                        E = chartDefaultOptions.legendDisplay
                            ? chartDefaultOptions.size.width / 1.8
                            : chartDefaultOptions.size.width / 2,
                        N = S.append('g').attr('transform', 'translate(' + E + ',' + chartDefaultOptions.size.height / 2 + ')'),
                        axisGrid = N.append('g').attr('class', 'axisWrapper'),
                        axisContainer = N.append('g').attr('class', 'axisContainer'),
                        blobWrapper = N.selectAll('.radarWrapper').data(f).enter().append('g').attr('class', 'radarWrapper');

                    axisGrid.selectAll('.levels').data(d3.range(1, chartDefaultOptions.levels + 1).reverse()).enter().append('circle').attr('class', 'gridCircle').attr('r', (function (t) {
                        return radius / chartDefaultOptions.levels * t;
                    })).style('fill', '#CDCDCD').style('stroke', '#CDCDCD').style('fill-opacity', chartDefaultOptions.colorOpacity.circle);

                    var axis = axisContainer.selectAll('.axis').data(allAxis).enter().append('g').attr('class', 'axis');

                    axis.append('line').attr('x1', 0).attr('y1', 0).attr('x2', (function (t, n) {
                    ...
                    })).attr('class', 'line').style('stroke', 'white').style('stroke-width', '2px'), axis.append('text').attr('class', 'legend').style('font-size', '14px').attr('text-anchor', 'middle').attr('dy', '0.35em').attr('x', (function (t, n) {
                        ...
                        t.each((function () {
                            for (var t, e = d3.select(this), r = e.text().split(/\s+/).reverse(), o = [], u = 0, a = e.attr('y'), c = e.attr('x'), l = parseFloat(e.attr('dy')), s = e.text(null).append('tspan').attr('x', c).attr('y', a).attr('dy', l + 'em'); t = r.pop();) {
                    ...
                    var radarLine = d3.lineRadial()
                        .curve(d3.curveLinearClosed).radius((function (t) {
                    ...
                    chartDefaultOptions.roundStrokes && radarLine.curve(d3.curveCardinalClosed), blobWrapper.append('path').attr('class', (function (t) {
                        return 'radarArea c' + fixText(t[0].radar_area);
                    })).attr('d', (function (t) {
                        return radarLine(t);
                    })).style('fill', (function (t, n) {
                        return chartDefaultOptions.color(n);
                    })).style('fill-opacity', chartDefaultOptions.colorOpacity.area).on('mouseover', (function () {
                        u()('#'.concat(chartContainerId)).css('cursor', 'pointer'), d3.selectAll('#'.concat(chartContainerId, ' .radarArea')).transition().duration(200).style('fill-opacity', chartDefaultOptions.colorOpacity.area_out), d3.select(this).transition().duration(200).style('fill-opacity', chartDefaultOptions.colorOpacity.area_over);
                    })).on('click', (function (t) {
                        var n = !1;
                        t.find((function (t) {
                            !0 === t.dim1IsNull && (n = !0);
                        })), n || !0 === _qlikUI.options.noInteraction || _qlikUI.backendApi.selectValues(0, [t[0].radar_area_id], !0);
                    })).on('mouseout', (function () {
                        u()('#' + chartContainerId).css('cursor', 'default'), d3.selectAll('#'.concat(chartContainerId, ' .radarArea')).transition().duration(200).style('fill-opacity', chartDefaultOptions.colorOpacity.area);
                    }))


                    blobWrapper.append('path').attr('class', 'radarStroke').attr('d', (function (t) {
                        return radarLine(t);
                    })).style('stroke-width', chartDefaultOptions.strokeWidth + 'px').style('stroke', (function (t, n) {
                        return chartDefaultOptions.color(n);
                    })).style('fill', 'none'), blobWrapper.selectAll('.radarCircle').data((function (t) {
                        return t;
                    })).enter().append('circle').attr('class', 'radarCircle').attr('r', chartDefaultOptions.dotRadius).attr('cx', (function (t, n) {
                        ...
                        return t;
                    })).enter().append('circle').attr('class', 'radarInvisibleCircle').attr('r', 1.5 * chartDefaultOptions.dotRadius).attr('cx', (function (t, n) {
                        return w(t.value) * Math.cos(angleSlice * n - Math.PI / 2);
                    })).attr('cy', (function (t, n) {
                        return w(t.value) * Math.sin(angleSlice * n - Math.PI / 2);
                    })).style('fill', 'none').style('pointer-events', 'all').on('mouseover', (function (t) {
                        if (!_qlikUI._inEditState) {
                            var e = parseFloat(d3.select(this).attr('cx')) - 10,
                                r = parseFloat(d3.select(this).attr('cy')) - 10;
                            L.attr('x', e).attr('y', r).text(t.radar_area + ' : ' + P(chartOptions.numberFormat[0], t.value * chartOptions.numberFormat[1]) + chartOptions.numberFormat[2]).transition().duration(200).style('opacity', 1);
                        }
                    })).on('mouseout', (function () {
                        _qlikUI._inEditState || L.transition().duration(200).style('opacity', 0);
                    }));
                    ...
                    if (N.selectAll('.axisLabel').data(d3.range(1, chartDefaultOptions.levels + 1).reverse()).enter().append('text').attr('class', 'axisLabel').attr('x', 4).attr('y', (function (t) {
                        ...
                        S.append('g').attr('class', 'legendOrdinal').attr('transform', 'translate(' + chartDefaultOptions.legendPosition.x + ',' + chartDefaultOptions.legendPosition.y + ')');

                        var T = d3.legend.color().shape('path', d3.svg.symbol().type('circle').size(40)()).shapePadding(10).scale(chartDefaultOptions.color).labels(chartDefaultOptions.color.domain().map((function (t) {
                            return f[t][0].radar_area;
                        }))).on('cellover', (function (t) {
                            !function (t) {
                                u()('#'.concat(chartContainerId)).css('cursor', 'pointer'), d3.selectAll('#'.concat(chartContainerId, ' .radarArea')).transition().duration(200).style('fill-opacity', chartDefaultOptions.colorOpacity.area_out), d3.select('#'.concat(chartContainerId, ' .c') + fixText(f[t][0].radar_area)).transition().duration(200).style('fill-opacity', chartDefaultOptions.colorOpacity.area_over);
                            }(t);
                        })).on('cellclick', (function (t) {
                            !function (t) {
                                u()('#' + chartContainerId).css('cursor', 'default');
                                var n = !1;
                                f[t].find((function (t) {
                                    !0 === t.dim1IsNull && (n = !0);
                                })), n || !0 === _qlikUI.options.noInteraction || (d3.selectAll('#'.concat(chartContainerId, ' .radarArea')).transition().duration(200).style('fill-opacity', .9), _qlikUI.backendApi.selectValues(0, [f[t][0].radar_area_id], !0));
                            }(t);
                        })).on('cellout', (function () {
                            u()('#' + chartContainerId).css('cursor', 'default'), d3.selectAll('#'.concat(chartContainerId, ' .radarArea')).transition().duration(200).style('fill-opacity', chartDefaultOptions.colorOpacity.area);
                        }));

                        1 !== qLayoutObj.qHyperCube.qDimensionInfo.length && 1 == chartDefaultOptions.legendDisplay && S.select('.legendOrdinal').call(T);
                    }
                } else {
                    !function (t) {
            ...
            var s = function (t, _qLayout) {
                const e = function (qLayout) {
                ...
                const radarOptions = {
                    ...
                    color: d3.scale.ordinal().range(r),
                    colorOpacity: {
                        circle: .1,
                        area: .2,
                        area_out: .1,
                    ...
                    roundStrokes: _qLayout.strokeStyle,
                    maxValue: .6,
                    levels: 6,
                    dotRadius: 4,
                    labelFactor: 1.02,
                    wrapWidth: 50,
                    strokeWidth: 2.8,
                    legendDisplay: _qLayout.showLegend,
                    numberFormat: function (t) {
                        var n = [];
                ...
                drawRadar('.radarChart', radarOptions, t, _qLayout, e, this);
            };
